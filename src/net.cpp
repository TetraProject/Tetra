...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Tetra " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
	{"tetra.blockexplorers.net", "tetra.blockexplorers.net"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Tetra is probably already running."), addrBind.ToString().c_str());
        else
